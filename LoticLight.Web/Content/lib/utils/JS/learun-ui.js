...
$.fn.ComboBox = function (options) {
    //options参数：description,height,width,allowSearch,url,param,data,onselect
    var $select = $(this);
    if (!$select.attr('id')) {
        return false;
    }
    ...
    if (options.url != undefined) {
        ...
        $option.find('li').click(function (e) {
            ...
            if (options.onselect != undefined) options.onselect($(this).attr('data-value'));
            e.stopPropagation();   
        }).hover(function (e) {
...
$.fn.SetWebControls = function (data) {
    ...
    for (var key in data) {
        ...
        {
            var _fildhtml = '<input type="hidden" id="' + key + '" value="' + data[key] + '" />';
            $("#hideFild").append(_fildhtml);
        }
    }
}
...
$.fn.authorizeButton = function () {
    ...
    if (data != undefined) {
        $.each(data, function (i) {
            $element.find("#" + data[i].ActionCode).attr('authorize', 'yes');
        });
    }
...
$.fn.authorizeColModel = function () {
    ...
    if (data != undefined) {
        $.each(data, function (i) {
            if (data[i].ViewCode.toLowerCase()== "id")
                return true;
            $element.showCol(data[i].ViewCode);
