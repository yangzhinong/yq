...
(function (factory) {
    ...
    function checkInput($input, $dropdownMenu, options) {
        if (!$dropdownMenu.length || // 过滤非 bootstrap 下拉式菜单对象
    ...
    function refreshDropMenu($input, data, options) {
        ...
        // 相同数据，不用继续渲染了
        if (options._lastData && JSON.stringify(options._lastData.value) === JSON.stringify(dataList) && $dropdownMenu.find('tr').length === len) {
            $dropdownMenu.show();
            adjustDropMenuPos($input, $dropdownMenu, options);
            return $input;
        }
        ...
        if (options.showHeader) {
            ...
            for (j in dataList[0]) {
                ...
                html.push('<th>', options.effectiveFieldsAlias[j] || j, index === 0 ? '(' + len + ')' : '', // 表头第一列记录总数
                '</th>');
        ...
        for (i = 0; i < len; i++) {
            ...
            html.push('<tr data-index="', dataI.__index || i, '" data-id="', idValue, '" data-key="', keyValue, '">', tds.join(''), '</tr>');
        }
        ...
        setTimeout(function () {
            ...
            if ($dropdownMenu.height() < $table.height() && Number($dropdownMenu.css('min-width').replace('px', '')) < $dropdownMenu.width()) {
                pdr = 18;
                mgb = 20;
            }
                ...
                if (!keyword) {
                    ...
                    for (i = 0; i < len; i++) {
                        for (key in data.value[i]) {
                            if (data.value[i][key] && isInWord(keyword, key, data.value[i][key] + '', options)) {
    ...
    var methods = {
        init: function init(options) {
            ...
            return self.each(function () {
                ...
                $input.on('keydown', function (event) {
                    ...
                    if ($.trim(word) && word === setOrGetAlt($input) && $dropdownMenu.find('table tr').length) {
        ...
        destroy: function destroy() {
            return this.each(function () {
                $(this).off().removeData(BSSUGGEST).removeAttr('style').parent().find('.btn:eq(0)').off().show().attr('data-toggle', 'dropdown').prop(DISABLED, FALSE) // .addClass(DISABLED);
