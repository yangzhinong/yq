...
///全局变量定义
var ClientDataUrl = "/Home/MenusData";///客户端获取数据地址
var MainPage = "/Home/Default";///默认页面地址
var PersonPage = "/PersonCenter/Index";//个人中心页面
var LoginOut = "/Login/LoginOut";//登出
var LoginPage = "/Login/Login";//登录页面


var tablist;//tabs列表
var authorizeMenuData = [];//菜单数据
var authorizeButtonData = [];//按钮权限
var authorizeColumnData = [];//视图权限
//var clientdataItem = [];
...
function initData() {
    $.ajax({
        url: contentPath + "/Home/MenusData",
        type: "post",
        dataType: "json",
        async: false,
        success: function (data) {
            authorizeMenuData = data.menus;//菜单
            authorizeButtonData = data.menusActions;//按钮
            authorizeColumnData = data.menusViews;//视图
        ...
        loadMenu: function () {
            ...
            $.each(data, function (i) {
                ...
                if (row.ParentId == "0") {
                    ...
                    _html += '<a href="#">'
                    _html += '<i class="fa ' + row.Icon + '"></i><span>' + row.FullName + '</span><i class="fa fa-angle-left pull-right"></i>'
                    _html += '</a>'
                    ...
                    if (childNodes.length > 0) {
                        ...
                        $.each(childNodes, function (i) {
                            ...
                            if (subchildNodes.length > 0) {
                                _html += '<a href="#"><i class="fa ' + subrow.Icon + '"></i>' + subrow.FullName + '';
                                _html += '<i class="fa fa-angle-left pull-right"></i></a>';
                                _html += '<ul class="treeview-menu">';
                                $.each(subchildNodes, function (i) {
                                    var subchildNodesrow = subchildNodes[i];
                                    _html += '<li><a class="menuItem" data-id="' + subchildNodesrow.ModuleId + '" href="' + subchildNodesrow.UrlAddress + '"><i class="fa ' + subchildNodesrow.Icon + '"></i>' + subchildNodesrow.FullName + '</a></li>';
                                });
                                _html += '</ul>';

                            } else {
                                _html += '<a class="menuItem" data-id="' + subrow.ModuleId + '" href="' + subrow.UrlAddress + '"><i class="fa ' + subrow.Icon + '"></i>' + subrow.FullName + '</a>';
                            }
                            _html += '</li>';
                        });
                        _html += '</ul>';
