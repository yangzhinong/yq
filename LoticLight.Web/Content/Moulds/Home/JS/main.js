...
function loadnav()
{
    ...
    tablist = $("#tab_list_add").Tab({
        items: [
            { id: 'desktop', title: '欢迎首页', closed: false, icon: 'fa fa fa-desktop', url: contentPath + MainPage },
        ],
        tabScroll: false,
    ...
    $("#UserSetting").click(function () {
        tablist.newTab({ id: "UserSetting", title: "个人中心", closed: true, icon: "fa fa fa-user", url: contentPath + PersonPage });
    });
    //动态加载导航菜单
    var _html = "";
    var index = 0;
    ...
    $("#nav").append(_html);
    $(".sub-nav-wrap").css('visibility', 'hidden');
    //$(".sub-nav-wrap").hide();
    ...
    $("#nav li.item").hover(function (e) {
        ...
        {
            $(this).find('.sub-nav-wrap').css('visibility', 'hidden');
            $(this).addClass('on');
