...
namespace LoticLight.Web.Areas.YQ.Controllers
{
    public class LocationController : BaseController
    {
        ...
        public ActionResult GridData(Pagination pagination, string keyword = "")
        {
            ...
            var data = Business.YqLocationService.Instance.LoadPagerEntities(
        ...
        public ActionResult SaveForm(Model.YqLocation obj)
        {
            if (string.IsNullOrEmpty(obj.Id))
            {
                obj.Id = Guid.NewGuid().ToString();
                Business.YqLocationService.Instance.AddEntities(obj);
            }
            else
            {
                Business.YqLocationService.Instance.UpdateEntities(obj);
            }
        ...
        {
            var data = Business.YqLocationService.Instance.FindEntities(keyValue);
            return Json(data, JsonRequestBehavior.AllowGet);
        }
        [SimpleCheckPermission("admin")]
        public ActionResult Delete(string keyValue)
        {
            var obj = Business.YqLocationService.Instance.FindEntities(keyValue.Trim());
            if (obj != null)
            {
                Business.YqLocationService.Instance.RemoveEntities(obj);
